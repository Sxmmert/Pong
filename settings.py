...
class Pong:
    ...
    def _ball_update(self):
        ...
        if self.ball.check_borders():
            ...
            border_sound = mixer.Sound('audio/wall.mp3')
            border_sound.play()
    ...
    def _reset(self):
        ...
        self.ball.direction_y = self.ball.random_ball_direction()
        sleep(1) # pauses the game for 1 second

    def _collission_ball_paddle(self):
        ''' Check for collission ball and paddle '''
        ball_left_paddle_collission = self._ball_left_paddle_collission()
        ball_right_paddle_collission = self._ball_right_paddle_collission()

        # when ball collides with paddle
        if ball_left_paddle_collission or ball_right_paddle_collission:
            # Play a sound
            paddle_sound = mixer.Sound('audio/paddle.mp3')
            paddle_sound.play()
    ...
    def _ball_left_paddle_collission(self):
        ''' Returns true if ball collides with left paddle '''
        if ((self.ball.ball_pos_x <= self.paddle_left.rect.right) and 
    ...
    def _ball_right_paddle_collission(self):
        ''' Returns true if ball collides with right paddle '''
        if ((self.ball.ball_pos_x <= self.paddle_right.rect.right) and 
    ...
    def _right_border_collission(self):
        ''' Left player scored '''
        goal_sound = mixer.Sound('audio/score.mp3')
        goal_sound.play()
